...
const monthlyExpense = async (req, res) => {
    try{
        ...
        if(isValidData){
            ...
            .then( async data => {
                if(data)
                    throw { errorMessage: 'Monthly income already exists'}
                return await  MonthlyExpense.create({ user_id, month : currentMonth,
...
const addExpense = async (req, res) => {
    try{
        ...
        if(isValidData){
            ...
            //checking wether payment method and category name is existing or not
            ...
            .then( async data => {
                ...
                return await DailyExpense.update({
                    dailyTotalExpense: isDataFound.dailyTotalExpense + amount
                },{ where: {
                    user_id, date_id
                }});
...
const monthlyTotalExpense = async (req, res) => {
    try {
        ...
        .then( data => {
            ...
            res.status(http.SUCCESS).json({'totalMonthExpense': totalMonthExpense});
        })
        .catch((err) =>{throw err})
    } catch(error) {
        res.status(http.BAD_REQUEST).json(error)
...
const editExpense = async (req, res) => {
    try {
        ...
        if(isValidData && currentMonth) {
            const {id, amount, category, paymentMethod, description} = req.body;
