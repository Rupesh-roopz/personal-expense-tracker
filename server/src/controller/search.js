const{ Sequelize, Date,
	ExpenseData, PaymentMethod, Category } = require('../models');
	...
	try {
		...
		if (isValidData) {
			...
			await Date.findAll({
				where : {
					user_id,
					date : { [Sequelize.Op.between] : [fromDate, endDate] }
				},
				attributes : ['date'],
				include : [
					{
...
const advancedSearch = async (req, res) => {
	try {
		...
		if(isValidData) {
			const { fromDate, endDate, categoryName,
				paymentMethod, priceRange } = req.body;
			const user_id = req.user_id;
			await Date.findAll({
				where : {
					user_id,
